...
void SettingsDialog::onRefreshRoomsClicked() {
    ...
    // Get grouped lights from backend
    QDBusMessage call = QDBusMessage::createMethodCall("org.kde.plasma.hue", "/org/kde/plasma/hue",
    ...
    if (reply.type() == QDBusMessage::ReplyMessage && !reply.arguments().isEmpty()) {
        ...
        // Get the argument - don't use const here
        QDBusArgument arg = reply.arguments().at(0).value<QDBusArgument>();

        qDebug() << "Got QDBusArgument, beginning array...";
        arg.beginArray();

        int count = 0;
        while (!arg.atEnd()) {
            qDebug() << "Processing item" << count++;
            QString id, name, type;
            arg.beginStructure();
            arg >> id >> name >> type;
            arg.endStructure();

            qDebug() << "  Item:" << id << name << type;
            ...
            if (count > 100) {
                qDebug() << "Breaking due to safety limit";
                break;
            }
        }

        qDebug() << "Finished processing array, endArray...";
        arg.endArray();
        qDebug() << "onRefreshRoomsClicked complete";
        ...
        qDebug() << "GetGroupedLights failed";
