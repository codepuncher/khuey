...
SettingsDialog::SettingsDialog(QWidget *parent)
    : QDialog(parent),
      dbusInterface(new QDBusInterface("org.kde.plasma.hue",
                                       "/org/kde/plasma/hue",
                                       "org.kde.plasma.hue",
...
void SettingsDialog::setupUI() {
    ...
    connect(subsampleSpinBox, QOverload<int>::of(&QSpinBox::valueChanged),
            subsampleSlider, &QSlider::setValue);
    
    QLabel *subsampleHint = new QLabel("Lower values = better performance, less color precision", subsampleGroup);
    ...
    // Info label
    QLabel *infoLabel = new QLabel("ℹ️  Note: Screen Sync must be restarted for changes to take effect.", syncTab);
    infoLabel->setWordWrap(true);
    infoLabel->setStyleSheet("QLabel { color: #3584e4; background-color: #e5f2ff; padding: 8px; border-radius: 4px; }");
    ...
    connect(refreshRoomsButton, &QPushButton::clicked, this, &SettingsDialog::onRefreshRoomsClicked);
    roomSelectLayout->addWidget(refreshRoomsButton);
    roomLayout->addLayout(roomSelectLayout);
    
    roomPreviewLabel = new QLabel("Select a room to control with power and brightness buttons", roomGroup);
    ...
    connect(testConnectionButton, &QPushButton::clicked, this, &SettingsDialog::onTestConnectionClicked);
    ...
    QLabel *connectionHint = new QLabel("💡 Tip: Edit ~/.openhue/config.yaml to change bridge IP or API key", bridgeGroup);
...
void SettingsDialog::saveSettings() {
    ...
    if (!syncReply.isValid() || !syncReply.value()) {
        QString errorMsg = syncReply.isValid() ? "Failed to save settings" : syncReply.error().message();
    ...
    if (!roomID.isEmpty()) {
        ...
        if (!roomReply.isValid() || !roomReply.value()) {
            QString errorMsg = roomReply.isValid() ? "Failed to save room" : roomReply.error().message();
            QMessageBox::warning(this, "Settings Error",
...
bool SettingsDialog::validateSettings() {
    ...
    if (subsample < 16 || subsample > 256) {
        QMessageBox::warning(this, "Validation Error", "Subsample width must be between 16 and 256.");
        tabWidget->setCurrentIndex(0); // Switch to Screen Sync tab
